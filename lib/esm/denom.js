...
const wormholeAssets = ((_assets_find = assets.find((x)=>x.chain_name === "gateway")) === null || _assets_find === void 0 ? void 0 : _assets_find.assets) || [];
...
const baseDenomToSymbol = (denom)=>{
    ...
    const baseDenom = denom.startsWith("ibc/") ? (_ibc_denom = ibc[denom]) === null || _ibc_denom === void 0 ? void 0 : _ibc_denom.base_denom : denom.startsWith("factory/") ? denom.split("/")[2] : denom.startsWith("factory:") ? denom.split(":")[2] : denom;
...
export class Denom {
    static from(string) {
        ...
        const bowUnderlying = (_Object_values_flatMap_find = Object.values(contracts).flatMap((x)=>x.bow).find((x)=>`factory/${x.address}/ulp` === string)) === null || _Object_values_flatMap_find === void 0 ? void 0 : (_Object_values_flatMap_find_config_denoms = _Object_values_flatMap_find.config.denoms) === null || _Object_values_flatMap_find_config_denoms === void 0 ? void 0 : _Object_values_flatMap_find_config_denoms.map(Denom.from);
        let osmosisUnderlying;
        if (string.startsWith("ibc/")) {
            var _osmosis_find_assets, _osmosis_find;
            const trace = ibc[string];
            osmosisUnderlying = (_osmosis_find = osmosis.find((x)=>`gamm/pool/${x.id}` === (trace === null || trace === void 0 ? void 0 : trace.base_denom))) === null || _osmosis_find === void 0 ? void 0 : (_osmosis_find_assets = _osmosis_find.assets) === null || _osmosis_find_assets === void 0 ? void 0 : _osmosis_find_assets.map((x)=>Denom.from(x.token.denom));
        }
...
const ibcDenom = (port, channel, denom)=>"ibc/" + Buffer.from(sha256(Buffer.from(`${port}/${channel}/${denom}`))).toString("hex").toUpperCase();
